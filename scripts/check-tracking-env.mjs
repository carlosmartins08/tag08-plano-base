...
for (const key of required) {
  const value = map.get(key) || process.env[key];
  if (!value) {
    missing.push(key);
    continue;
  }
...
const ga = map.get('NEXT_PUBLIC_GA_TRACKING_ID') || process.env.NEXT_PUBLIC_GA_TRACKING_ID || '';
const gt = map.get('NEXT_PUBLIC_GOOGLE_TAG_ID') || process.env.NEXT_PUBLIC_GOOGLE_TAG_ID || '';
const ads = map.get('NEXT_PUBLIC_GOOGLE_ADS_ID') || process.env.NEXT_PUBLIC_GOOGLE_ADS_ID || '';
const pixel = map.get('NEXT_PUBLIC_META_PIXEL_ID') || process.env.NEXT_PUBLIC_META_PIXEL_ID || '';
